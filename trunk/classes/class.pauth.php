...
class pauth {
	...
	*/
	...
	function setpAuth(){
		...
		setcookie("id_".$this-> user['section'],$this-> user['id'],$this-> user['time'] + $this-> cookie['expires']);
		setcookie("user_".$this-> user['section'],$this-> user['login'],$this-> user['time'] + $this-> cookie['expires']);
		setcookie("section_".$this-> user['section'],$this-> user['section'],$this-> user['time'] + $this-> cookie['expires']);
	...
	*/
	...
	function valExp($id,$section){
		...
		if($ftime + $this-> cookie['expires'] < $time){
			...
			//delete cookies
			setcookie("id_".$section,"",time()-3600);
			setcookie("user_".$section,"",time()-3600);
			setcookie("section_".$section,"",time()-3600);
