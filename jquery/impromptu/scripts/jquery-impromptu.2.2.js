...
jQuery.extend({	
	ImpromptuDefaults: { prefix:'jqi', buttons:{ Ok:true }, loaded:function(){}, submit:function(){return true;}, callback:function(){}, opacity:0.6, zIndex: 999, overlayspeed:'slow', promptspeed:'fast', show:'fadeIn', focus:0, useiframe:false, persistent: true },
	...
	prompt: function(m,o){
		...
		jQuery.each(m,function(statename,stateobj){
			...
			state.children('.'+ o.prefix +'buttons').children('button').click(function(){
				...
				jqi.find('#'+ o.prefix +'states :input').each(function(i,el){
					var n = jQuery(el).attr('name');
					forminputs[n] = jqi.find("#"+ o.prefix +"states :input[name='"+ n +"']").val();
				});
		...
		var positionPrompt = function(){
			...
			jqi.css({ position: "absolute", top: (w.height()/5), left: "50%", marginLeft: ((jqi.outerWidth()/2)*-1) });					
		};
